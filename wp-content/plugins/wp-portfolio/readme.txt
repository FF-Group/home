...
The plugin requires you to have an account with [Shrink The Web](http://www.shrinktheweb.com/) if you wish to generate the thumbnails **dynamically**. Please read [the first FAQ about account types](http://wordpress.org/extend/plugins/wp-portfolio/faq/) to learn more.  
...
please raise a problem in the [WP Portfolio Support Forum on Wordpress.org](http://wordpress.org/tags/wp-portfolio?forum_id=10), and we'll respond to the ticket as soon as 
...
Yes. To add custom fields you will need to add to `wp-content/themes/%YOUR_THEME%/functions.php` (where `%YOUR_THEME% is the name of the wordpress theme you are using),
here you should use add_filter to add a function to 'wpportfolio_filter_portfolio_custom_fields'. Your function will need to return an array containing an associative array for each item, for example:
`
...
`
The name and template tag properties are required, without them we can't generate your custom fields and
though not compulsory we highly recommend adding a label and description to guide additions.
